package game.nd.builder

import org.hexworks.zircon.api.TileColors

object GameColors {

    val WALL_FOREGROUND = TileColors.fromString("#75715E")
    val WALL_BACKGROUND = TileColors.fromString("#3E3D32")
    val WALL_FOREGROUND_MOSSY = TileColors.fromString("#607649")
    val WALL_BACKGROUND_MOSSY = TileColors.fromString("#2a3726")


    val FLOOR_FOREGROUND = TileColors.fromString("#75715E")
    val FLOOR_BACKGROUND = TileColors.fromString("#000000")

    val ACCENT_COLOR = TileColors.fromString("#FFCD22")

    val FUNGUS_COLOR = TileColors.fromString("#85DD1B")
    val BAT_COLOR = TileColors.fromString("#2348b2")
    val ZOMBIE_COLOR = TileColors.fromString("#618358")

    val ROCK_COLOR = TileColors.fromString("#dddddd")
    val MUSHROOM_COLOR = TileColors.fromString("#E4D8CA")
    val BAT_MEAT_COLOR = TileColors.fromString("#EA4861")
    val METAL_COLOR = TileColors.fromString("#99aabb")
    val STAFF_COLOR = TileColors.fromString("#964B00")

    val UNREVEALED_COLOR = TileColors.fromString("#000000")
    val OUT_OF_LOS_COLOR = TileColors.fromString("#444444")
}